...
def register():
    ...
    if request.method=='POST':
        ...
        # Execute query
        cur.execute("INSERT INTO admins(id, name, email, username, password) VALUES(%s, %s, %s, %s, %s)", (str(len(rows)+1), name, email, username, password))
...
def bookings(id):
    ...
    if request.method=='POST':#form.g_id.validate_on_submit():
        ...
        if id[0] == 'R':
            ...
            result = cur.execute("SELECT cost FROM charges WHERE code = 2 AND type=%s",[f_type])
            result = cur.fetchone()
            f_cost = result['cost']
        ...
        if id[0] == 'R':
            ...
            cur.execute("INSERT INTO bookings(b_id, r_id, g_id, b_status, a_id, st, et, f_type, f_cost) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s)", (b_id, id, g_id, status, '0', check_in, check_out, f_type, f_cost*day))
        
        else:
            cur.execute("INSERT INTO bookings(b_id, r_id, g_id, b_status, a_id, st, et, f_type, f_cost) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s)", (b_id, 'R100', g_id, status, id, check_in, check_out, f_type, f_cost))
...
@app.route('/delete_guest/<string:id>', methods=['GET', 'POST'])
@is_logged_in
def delete_guest(id):
    cur=mysql.connection.cursor()
    cur.execute("DELETE FROM bookings WHERE g_id=%s", [id])
    ...
    flash('Succesful checkout', 'success')
...
def generate_bill(id):
    ...
    return render_template('generate_bill.html', len=len(bookings), guest=guest, bookings=bookings, total=total)
